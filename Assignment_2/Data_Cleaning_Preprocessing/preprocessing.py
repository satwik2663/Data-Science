...
def data_cleaning():
    ...
    s3 = boto3.client('s3', aws_access_key_id=aws_id, aws_secret_access_key=aws_secret)
    obj = s3.get_object(Bucket='data-science-team1', Key='Final100Keywords.csv')
    data = obj['Body'].read()
    ...
    clean_word_list = []
    for key, i in word_list['keyword'].iteritems():
        a = re.sub('[^A-Za-z]+', ' ', str(i))
        a = a.lower()
    ...
    # with_count_data_frame.to_csv("with_count_data_frame.csv")

    client = boto3.client('s3', aws_access_key_id='AKIAIBIGY3IWD3P7RAHA', aws_secret_access_key='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')
    transfer = S3Transfer(client)
    transfer.upload_file('with_count_data_frame.csv', 'data-science-team1', 'with_count_data_frame')
